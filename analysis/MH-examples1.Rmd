...
```{r}
set.seed(123)
z1=easyMCMC(1000,3,0.01) # step is too small, stuck local
z2=easyMCMC(1000,3,1) # just right
z3=easyMCMC(1000,3,100) # step is too large, and mostly rejected
...
```{r}
target = function(x){
  # added a 1e-10 for numerical stability
  return((x>0 & x <1) + (x>2 & x<3)+1e-10) 
}
...
y=target(x)
plot(x,y,type="l")
# target has two modes
```

What happens if the proposal sd is too small here? (try e.g. 1 and 0.1)
```{r}
set.seed(123)
z1=easyMCMC(1000,3,0.1) # step is too small, stuck local based on starting point
z2=easyMCMC(1000,3,1) # just right
z3=easyMCMC(1000,1,0.1) # step is too small, stuck local based on starting point
...
```{r}
z=psampler(nAA = 50,nAa = 21,naa=29,niter=10000,pstartval=0.5,pproposalsd=0.1)
x=seq(0,1,length=1000)
hist(z,prob=T)
lines(x,dbeta(x,122, 80))  # overlays beta density on histogram
 

z=psampler(nAA = 50,nAa = 21,naa=29,niter=10000,pstartval=0.5,pproposalsd=0.001) # too small, stuck local
hist(z,prob=T)
lines(x,dbeta(x,122, 80))  # overlays beta density on histogram
```
